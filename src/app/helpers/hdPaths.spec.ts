...
describe('Helpers: hdPaths', () => {
  ...
  it('pathToObject', () => {
    ...
    expect(pathObject).toEqual({m: 'm', purpose: '44', coinType: '0', account: '0', change: '0', index: '0'});
  });
  ...
  it('getHdIndexPaths - pagination by 10', () => {
    ...
    Array.from(Array(10).keys()).map(index => {
      expect(hdPaths[index]).toEqual(`m/44'/0'/${index + 10}'/${index + 10}/${index + 10}`);
    });
  ...
  it('composePath', async () => {
    ...
    path = hdPathsHelper.composePath({m: 'x', purpose: '20', coinType: '12', account: '01'});
    expect(path).toEqual(`x/20'/12'/01'/0/0`);

    path = hdPathsHelper.composePath({m: 'x', purpose: '20', coinType: '12', account: '01', change: '1'});
    expect(path).toEqual(`x/20'/12'/01'/1/0`);

    path = hdPathsHelper.composePath({m: 'x', purpose: '20', coinType: '12', account: '01', change: '1', index: '10'});
    expect(path).toEqual(`x/20'/12'/01'/1/10`);

    path = hdPathsHelper.composePath({m: 'm', purpose: '44', coinType: '0', account: '0', change: '0', index: '4'});
