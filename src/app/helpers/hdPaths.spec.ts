...
describe('Helpers: hdPaths', () => {
  ...
  it('getNextPath', () => {
    const wallets = [{ basePath: `m/44'/60'/0'/0/0`, main: true }];
    let hdPath = hdPathsHelper.getNextPath(wallets);
    expect(hdPath).toEqual(`m/44'/60'/0'/0/1`);

    wallets.push({ basePath: `m/44'/60'/0'/0/1`, main: false });
    wallets.push({ basePath: `m/44'/60'/0'/0/2`, main: false });

    hdPath = hdPathsHelper.getNextPath(wallets);
    expect(hdPath).toEqual(`m/44'/60'/0'/0/3`);
  });

  it('getHdMainPath', () => {
    const hdPath = hdPathsHelper.getHdMainPath({});
    expect(hdPath).toEqual(`m/44'/60'/0'/0/0`);
  });
  ...
  it('getHdIndexPaths - pagination by 10', () => {
    ...
    Array.from(Array(10).keys()).map((index) => {
      expect(hdPaths[index]).toEqual(`m/44'/60'/${index}'/${index}/${index}`);
    });
    ...
    Array.from(Array(10).keys()).map((index) => {
      expect(hdPaths[index]).toEqual(
        `m/44'/60'/${index + 10}'/${index + 10}/${index + 10}`
      );
    });
  ...
  it('composePath', async () => {
    ...
    expect(path).toEqual(`x/44'/60'/0'/0/0`);

    path = hdPathsHelper.composePath({ m: 'x', purpose: '20' });
    expect(path).toEqual(`x/20'/60'/0'/0/0`);
