...
export const notEqual = (firstControlName: string, secondControlName: string): ValidatorFn =>
  (formGroup: FormGroup): { [key: string]: any } | null => {
    ...
    if (firstControl && secondControl) {
      return firstControl.value === secondControl.value ? {equal: true} : null;
    }
...
export class NotEqualValidator implements Validator {
  ...
  validate(control: FormGroup): { [key: string]: any } | null {
    ...
    return this.matchControls ? notEqual(controlsArray[0], controlsArray[1])(control) : null;
  }
