...
const JsonTest = {
  ...
  compareKeys(baseLan, otherLang) {
    ...
    return jsonDiff.diff(JSON.parse(baseLan.content), JSON.parse(otherLang.content), {keysOnly: true});
  ...
  async getFiles() {
    return new Promise((resolve) => {
      fs.readdir(path.resolve(__dirname, '../src/assets/i18n'), (err, filenames) => {
        if (err) {
          return;
        }
        resolve(filenames);
  ...
  async readFile(file) {
    return new Promise(async (resolve) => {
      fs.readFile(path.resolve(__dirname, '../src/assets/i18n/' + file), 'utf-8', (err, content) => {
        if (err) {
          return;
        }
        resolve(content);
  ...
  validateFile() {
    ...
    const filesToCompare = JsonTest.files.filter(w => w.name !== JsonTest.baseFile);
